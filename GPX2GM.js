...
var GPX2GM = GPX2GM || (function() {
	...
	var language = normaliseLang(navigator.language /* Mozilla */ || navigator.userLanguage /* IE */),
		...
		'en': {
			...
			'H<br />�<br />h<br />e<br />&nbsp;<br />in<br />&nbsp;<br />': 'E<br />l<br />v<br />.<br />&nbsp;<br />in<br />&nbsp;<br />',
			...
			'Ihr Browser unterst�tzt nicht die ben�tigten Methoden!': 'Your browser doesn\'t support the methods needed by Google maps.'
			}
		},
		unit_conversions = {
			'us': {
	...
	function makeMap(ID) {
		...
		var hp_ytext = _("H<br />�<br />h<br />e<br />&nbsp;<br />in<br />&nbsp;<br />") + _u('m');
	...
	//  map.addMapType(G_SATELLITE_3D_MAP); // ben�tigt Plugin
		...
		if(hp) {
			...
			JB_GM_Info(id,"H�henprofil, ID: "+id_hp,false);
		}
		...
		this.parseGPX = function(data) {
			...
			for(var k=0;k<trk.length;k++) {
				...
				if(hflag) JB_GM_Info(id,"H�hendaten gefunden",false); else JB_GM_Info(id,"Keine H�hendaten gefunden",false);
		...
		this.markermove = function(p,a) {
			...
			if(hp) {
				info += "<br />" + _('H�he') + ":&nbsp;"+_u('m', Math.round(a.h), 0, true);
